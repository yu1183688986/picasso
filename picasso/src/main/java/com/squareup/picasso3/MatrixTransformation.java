...
public class MatrixTransformation implements Transformation {
  ...
  @Override public RequestHandler.Result transform(RequestHandler.Result source) {
    ...
    Bitmap bitmap = transformResult(data, source.getBitmap(), source.getExifOrientation());

    return new RequestHandler.Result(bitmap, source.getLoadedFrom(), source.getExifOrientation());
  }
